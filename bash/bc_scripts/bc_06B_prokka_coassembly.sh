...
#SBATCH --job-name="bc_06B_prokka_coassembly"
#SBATCH -A b1042
...
#SBATCH --mem=200Gb
#SBATCH -t 24:00:00
#SBATCH --output=bc_06B_prokka_coassembly.out
#SBATCH --error=bc_06B_prokka_coassembly.err
#SBATCH -p genomics
...
cd /projects/b1052/Zhen/pdna_meta/bc_04_metabat_coassembly/3aBC/
for F in *.fa; do
  N=$(basename $F .fa) ;
...
cd /projects/b1052/Zhen/pdna_meta/bc_04_metabat_coassembly/9aBC/
for F in *.fa; do
  N=$(basename $F .fa) ;
  prokka $F --outdir $N --prefix $N --metagenome --cpus 12;
done
